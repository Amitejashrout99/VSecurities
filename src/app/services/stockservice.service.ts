...
export class StockserviceService 
{
  ...
  {
    return this.http_service_stock.get<stock[]>('allStocks')
  ...
  {
    return this.http_service_stock.post<stock_watchlist>("addStockWatchlist",partcular_stock);
  }

  getAllWatchlistedStocks():Observable<stock_watchlist[]>
  {
    return this.http_service_stock.get<stock_watchlist[]>("allWatchlistedStocks")
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  async checkWatchlistStatus(stock_id:number):Promise<stock_watchlist>
  {
    return this.http_service_stock.get<stock_watchlist>("checkWatchListStatus/"+stock_id)
    .pipe(catchError(this.error_management_service.handle_error_faced)).toPromise();
  }

  removeStockFromWatchlist(watchlist_id:number):Observable<string>
  {
    return this.http_service_stock.delete<string>("deleteWatchlistedStock/"+watchlist_id)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }
  ...
  {
    return this.http_service_stock.get<stock_sales[]>("getBoughtStocks/"+user_id,{observe:'response'})
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  sellStock(transaction_id:number,sell_stock_object:stock_sales):Observable<stock_sales>
  {
    return this.http_service_stock.put<stock_sales>("updateStockSaleData/"+transaction_id,sell_stock_object)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  async getParticularBoughtStockData(transaction_id:number):Promise<stock_sales>
  {
    return this.http_service_stock.get<stock_sales>("getParticularStockSaleData/"+transaction_id)
    .pipe(catchError(this.error_management_service.handle_error_faced)).toPromise();
  }

  getAllBoughtStocks(user_id:number):Observable<HttpResponse<stock_sales[]>>
  {
    return this.http_service_stock.get<stock_sales[]>("getAllBoughtStocks/"+user_id,{observe:'response'})
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  getAllSoldStocks(user_id:number):Observable<HttpResponse<stock_sales[]>>
  {
    return this.http_service_stock.get<stock_sales[]>("getAllSoldStocks/"+user_id,{observe:'response'})
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }
  ...
  {
    return this.http_service_stock.get<stock_reviews[]>("BoughtStockReviews/"+bought_stocks_ids)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  getPendingReviewsStockIds(bought_stock_ids:number[],user_id:number):Observable<number[]>
  {
    return this.http_service_stock.get<number[]>("getPendingReviewsForUser/"+user_id+"/"+bought_stock_ids)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  ...
  {
    return this.http_service_stock.post<stock_reviews>("submitStockReview",bought_stock_review)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }

  postUserReviewTrack(user_review_data:user_pending_reviews_track):Observable<user_pending_reviews_track>
  {
    return this.http_service_stock.post<user_pending_reviews_track>("insertStockReviewTrackData",user_review_data)
    .pipe(catchError(this.error_management_service.handle_error_faced));
  }
